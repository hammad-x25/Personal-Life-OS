...
import { dashboardToday, dashboardPeriod, growth } from '../services/analytics.service.js';
import { generateReview } from '../services/ai.service.js';
import { buildReviewPayload } from '../services/review.service.js';
...
export const generate = type => async (req, res) => {
  ...
  const dashboard = type === 'DAILY' ? await dashboardToday(req.user) : await dashboardPeriod(req.user, type);
  ...
  const payload = buildReviewPayload({ type, period, score: type === 'DAILY' ? dashboard.score.score : dashboard.score ?? comparison.currentScore, growthPercentage: comparison.growthPercentage, dashboard });
