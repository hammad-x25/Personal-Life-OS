...
export const markAllRead = async (req, res) => { await Notification.updateMany({ userId: req.user._id, readAt: null, status: { $ne: 'DISMISSED' } }, { $set: { readAt: new Date(), status: 'SENT' } }); return ok(res, null, 'Notifications marked as read'); };
